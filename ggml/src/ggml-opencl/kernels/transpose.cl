...
kernel void kernel_transpose_32_16(__read_only image1d_buffer_t input, 
                                        __write_only image1d_buffer_t output, 
                                        const uint rows, 
                                        const uint cols, 
                                        const uint padded_rows) {
    
    const uint i = get_global_id(0); // 0..cols-1 (texel = float4/half4 1개)
    const uint j = get_global_id(1); // 0..rows-1
    if (i >= cols || j >= rows) return;

    const uint coord = j*cols + i;
    half4 h = read_imageh(input, coord);
    write_imageh(output, coord, h);
}