...
kernel void kernel_mul_mat_Ab_Bi_8x4(
    __global const ushort *src0_q,          // Q4_0 weights   [M x K/2], 4bit packed (nibbles)
    __global const half   *src0_d,          // scales         [M x (K/32)]
    __read_only  image1d_buffer_t src1,     // B: N x K row-major, pixel = half4 (K/4 픽셀)
    __global float *dst,                    // C: N x M, row-major
    int ne01,                               // M
    ...
    int ne1)                                // N
{
    ...
    __global const ushort *wptr = src0_q + oc4;
    __global const half   *sptr = src0_d + oc4;
