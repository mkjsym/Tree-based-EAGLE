...
std::vector<size_t> TopK(const std::vector<float>& data, size_t k) {
    size_t n = data.size();
    // k가 데이터 크기보다 크면 데이터 크기로 조정
    if (k > n) {
        k = n;
    }

    // 0, 1, 2, ..., n-1 로 초기화된 인덱스 벡터 생성
    std::vector<size_t> indices(n);
    std::iota(indices.begin(), indices.end(), 0);

    // 부분 정렬: indices의 첫 k개 원소만 정렬된 상태로 만듦.
    // std::partial_sort(시작, k번째 위치, 끝, 비교함수)
    std::partial_sort(
        indices.begin(), 
        indices.begin() + k, // 여기까지 정렬
        indices.end(),
    ...
    // 정렬된 상위 k개의 인덱스만 남기기 위해 벡터 크기를 줄임
    indices.resize(k);
...
int main(int argc, char ** argv) {
    ...
    // LOG("Initialize Auccumulated Probability Table: \n");
    ...
    while (true) {
        ...
        for (int i = 0; i < n_draft; ++i) {
            ...
            for (int s = 0; s < n_seq_dft; ++s) {
                ...
                for (int f = 1; f < expand_k; ++f) {
                    ...
                    if (n_seq_cur < n_seq_dft) {
                        LOG("splitting seq %3d into %3d, drafts[n_seq_cur].i_batch_dft = %d, drafts[s].i_batch_dft = %d\n", s, n_seq_cur, drafts[n_seq_cur].i_batch_dft, drafts[s].i_batch_dft);
            ...
            for (int is = 0; is < (int) ids.size(); ++is) {
                ...
                if (topk_indices.size() == 1) {
                    common_batch_add(batch_dft, id, n_past_cur, {s}, true);
                    LOG("Adding token %d ('%s') for sequence %d to draft batch\n", id, common_token_to_piece(ctx_dft, id).c_str(), s);
                    temp.insert(temp.end(), cb_data.data.begin() + (4096 * temp_i_batch_dft[s]), cb_data.data.begin() + (4096 * (temp_i_batch_dft[s] + 1)));
                    LOG_DBG("s*4096=%d, (s+1)*4096=%d\n", (4096 * temp_i_batch_dft[s]), (4096 * (temp_i_batch_dft[s] + 1)));
                }
                else {
            ...
            LOG("cb_data.data.size(): %zu\n", cb_data.data.size());
        }

        /////////////////////////////////////////Tree Decoding End///////////////////////////////////////

        /////////////////////////////////////////Drafting End///////////////////////////////////////
        LOG("// Target: batch_tgt.n_tokens: %d\n", batch_tgt.n_tokens);
        ...
        // evaluate the target model on the drafted tokens
        {
            ...
            LOG("/////////////////////////////batch_tgt.n_tokens: %d, target model decoding took %.3f seconds\n", batch_tgt.n_tokens, (t_dec_end - t_dec_start) / 1e6f);
    ...
    // LOG_INF("Verification/Draft Count: %zu", verification_latencies.size());
    }
