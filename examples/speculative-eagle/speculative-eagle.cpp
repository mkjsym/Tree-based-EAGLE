//Tree-based EAGLE 구현 코드 (ksc 2025 13)
...
int main(int argc, char ** argv) {
    ...
    int third_depth[4] = { 0, 4, 5}; // 각 깊이별로 몇 개의 시퀀스가 있는지 저장 -ym-
    ...
    while (true) {
        ...
        for (int i = 0; i < n_draft; ++i) {
            ...
            LOG("batch_tgt.n_tokens: %d, n_draft: %d\n", batch_tgt.n_tokens, n_draft);
            if (batch_tgt.n_tokens >= n_draft) {
                break;
            }
...
//                     common_batch_add(batch_tgt, id, n_past_tgt + i + 1, { s }, true);
//                     LOG("added to target batch: s=%d, id=%d\n", s, id);
//                     LOG("batch_tgt.n_tokens: %d\n", batch_tgt.n_tokens);
...
//                         if (s == 0) {
//                             common_batch_add(batch_dft, id, n_past_cur, { s }, true);
//                             LOG("cur_depth 0 added: s=%d, id=%d\n", s, id);
//                         }
